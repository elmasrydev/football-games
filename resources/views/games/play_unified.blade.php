...
<div class="max-w-[1440px] mx-auto px-4 sm:px-8 space-y-6 pb-20" style="--genre-theme: {{ $themeColor }};">
    ...
    <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 py-4">
        <div class="space-y-4">
            <div class="flex flex-wrap items-center gap-3">
                <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[var(--genre-theme)]/10 border border-[var(--genre-theme)]/20 text-[var(--genre-theme)] text-[10px] font-display font-black uppercase tracking-[0.2em]">
        ...
        <div class="glass-card flex items-center justify-between p-2 rounded-2xl border-outline-variant/20 min-w-[320px]">
            @if($currentLevel > 1)
                <a href="{{ route('games.play', ['slug' => $game->slug, 'genre' => $selectedGenre?->slug, 'level' => $currentLevel - 1]) }}" 
                   class="w-10 h-10 rounded-xl bg-surface-variant/50 flex items-center justify-center text-on-surface-variant hover:text-[var(--genre-theme)] transition-colors active:scale-90">
            ...
            <div class="flex items-center gap-3 px-4">
                ...
                <input type="number" id="level-input" value="{{ $currentLevel }}" min="1" max="{{ $totalChallenges }}" 
                       ...
                       class="w-16 bg-surface-variant/50 border border-outline-variant/30 rounded-lg text-center font-display font-black text-[var(--genre-theme)] py-1 focus:outline-none focus:border-[var(--genre-theme)] transition-all">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-8 xl:col-span-9 space-y-8">
            ...
            <div class="glass-card rounded-[2.5rem] overflow-hidden relative border-outline-variant/10 shadow-2xl">
                ...
                <div class="absolute top-6 end-6 z-20 flex flex-col gap-3">
                    <button class="w-12 h-12 rounded-full glass-card flex items-center justify-center text-on-surface-variant hover:text-[var(--genre-theme)] shadow-lg hover:scale-110 transition-all active:scale-90"
            ...
            <!-- Answer Block -->
            <div class="glass-card rounded-[2.5rem] p-6 sm:p-8 border-[var(--genre-theme)]/20 shadow-xl relative overflow-hidden bg-gradient-to-b from-[var(--genre-theme)]/5 to-transparent">
                <!-- Cyber Background Detail -->
                <div class="absolute top-0 right-0 w-64 h-64 bg-[var(--genre-theme)]/5 rounded-full blur-3xl -mr-32 -mt-32"></div>
                
                <div class="relative z-10 space-y-6 max-w-3xl mx-auto">
                    @if($game->slug === 'terminology-trivia')
                    <div class="absolute top-0 end-0">
                        <button class="w-10 h-10 rounded-full glass-card flex items-center justify-center text-on-surface-variant hover:text-[var(--genre-theme)] shadow-lg hover:scale-110 transition-all active:scale-90"
                                onclick="toggleBookmark({{ $game->id }})" 
                                data-id="{{ $game->id }}">
                            <span class="material-symbols-outlined transition-colors">bookmark</span>
                        </button>
                    ...
                    <div class="text-center space-y-1">
                        <span class="text-[9px] font-display font-black text-[var(--genre-theme)] uppercase tracking-[0.4em] block">{{ __('Interaction') }}</span>
                    ...
                    @if($game->slug === 'anagram-arena' || $game->slug === 'vowel-void')
                        <div class="mt-6 p-6 rounded-3xl bg-surface-variant/20 border border-outline-variant/10 text-center">
                            <span class="text-[9px] font-display font-black text-on-surface-variant/40 uppercase tracking-[0.4em] block mb-3">{{ __('Word Structure') }}</span>
                            <div class="flex flex-wrap justify-center gap-4 text-2xl font-display font-black text-[var(--genre-theme)] tracking-[0.3em]" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
                                @php
        ...
        <div class="lg:col-span-4 xl:col-span-3 space-y-6">
            ...
            <div class="glass-card rounded-[2rem] overflow-hidden flex flex-col border-[var(--genre-theme)]/10 bg-gradient-to-br from-[var(--genre-theme)]/5 to-transparent">
                <div class="p-4 bg-[var(--genre-theme)]/5 border-b border-[var(--genre-theme)]/10 flex items-center gap-3">
                    <span class="text-xl">📖</span>
                    <h3 class="text-xs font-display font-black text-on-surface uppercase tracking-widest">{{ __('How to Play') }}</h3>
                </div>
                <div class="p-6">
                    <div class="prose prose-xs dark:prose-invert text-on-surface-variant font-medium leading-relaxed opacity-80">
            ...
            <div class="glass-card rounded-[2rem] overflow-hidden flex flex-col border-outline-variant/10">
                ...
                <div class="p-6 space-y-6">
                    <div class="text-center space-y-4">
                        ...
                        <button id="hint-btn" class="w-full py-4 bg-[var(--genre-theme)] text-on-primary rounded-2xl font-display font-black uppercase tracking-widest text-xs hover:brightness-110 transition-all shadow-lg shadow-[var(--genre-theme)]/20 active:scale-95">
                            {{ __('Unlock Hint') }}
                        </button>
                    </div>
            ...
            <!-- Ad/Promo Spot -->
            <div class="glass-card rounded-[2rem] p-6 border-[var(--genre-theme)]/10 bg-gradient-to-br from-[var(--genre-theme)]/5 to-transparent">
                <span class="text-[9px] font-display font-black bg-[var(--genre-theme)] text-on-primary px-2 py-0.5 rounded-full uppercase tracking-widest mb-3 inline-block">{{ __('Pro Benefit') }}</span>
...
<script>
    ...
    function highlightSuccess() {
        ...
        if (arena) {
            ...
            arena.style.borderColor = 'var(--genre-theme)';
        }
    }
