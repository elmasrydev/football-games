...
@section('content')
    <div class="container play-container">
        <div class="word-section">
            <div class="puzzle-box">
                <div class="glossary-clue">
                    <p>{{ $challenge->clue }}</p>
                </div>
            </div>

            <div class="word-description">
                <p>What is the correct football term for this description?</p>
            </div>

            <div class="controls">
                <a href="{{ route('games.glossary.play') }}" class="btn btn-outline">
                    Try Another Term
                </a>
        ...
        <div class="interaction-section">
            <div class="question-card">
                <div class="question-header">
                    <h3>📖 Football Glossary</h3>
                    <span class="difficulty-badge {{ $challenge->difficulty }}">
                        {{ ucfirst($challenge->difficulty) }}
                    </span>
                </div>
                <div class="question-body">
                    <p>Enter the term below</p>
                </div>

                <x-player-answer-form placeholder="Correct term..." />
    ...
    @push('styles')
        <style>
            ...
            .glossary-clue { text-align: center; }
            .glossary-clue p { font-size: 2rem; font-weight: 700; color: var(--pitch-dark); line-height: 1.4; }
    ...
    @push('scripts')
        <script>
            ...
            document.getElementById('submit-btn').addEventListener('click', () => {
                ...
                fetch(`/glossary/${challengeId}/check`, {
                    method: 'POST',
            ...
            document.getElementById('hint-btn').addEventListener('click', () => {
                fetch(`/glossary/${challengeId}/hint`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken },
            ...
            document.getElementById('give-up-btn').addEventListener('click', () => {
                fetch(`/glossary/${challengeId}/reveal`)
                .then(r => r.json())
                .then(data => {
                    const feedback = document.getElementById('feedback');
