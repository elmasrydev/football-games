    ...
    <div class="max-w-[1200px] mx-auto">
        ...
        <div class="text-center mb-12">
            ...
            <h1 class="text-5xl sm:text-6xl font-display font-black uppercase tracking-tight mb-4">
                <span class="bg-gradient-to-r from-primary via-secondary to-tertiary bg-clip-text text-transparent">{{ __('Mazad') }}</span>
                <span class="text-on-surface"> مزاد</span>
            </h1>
            <p class="text-on-surface-variant text-lg max-w-xl mx-auto leading-relaxed">
                {{ __('Compete in real-time! Answer as many correct answers as possible before time runs out.') }}
            </p>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function fetchRooms() {
        try {
            const res = await fetch(`/${locale}/mazad/rooms`, {
    ...
    async function joinRoom(code) {
        ...
        try {
            const res = await fetch(`/${locale}/mazad/rooms/${code}/join`, {
                method: 'POST',
                headers: {
            ...
            if (res.ok) {
                window.location.href = `/${locale}/mazad/room/${code}`;
            } else {
    ...
    document.getElementById('create-room-form')?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                window.location.href = `/${locale}/mazad/room/${result.room.code}`;
            } else {
                const err = await res.json();
