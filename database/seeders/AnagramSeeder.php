...
class AnagramSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $egyActors = [
            ...
            ['Nelly', 'Fawazir legend', 'Sister of Feyrouz'],
    ...
    {
        // Mapping genre slug to autocomplete_type
        $autocompleteMapping = [
            'football' => 'player',
            'actors' => 'actor',
            'movies' => 'movie',
        ...
        foreach ($data as $item) {
            ...
            // Sync with GameItems to ensure autocomplete works
            $gameItem = GameItem::where('name_en', $answer)
                ->where('type', $autocompleteType)
                ->first();

            if (!$gameItem) {
                $gameItem = GameItem::create([
                    'name_en' => $answer,
                    'name_ar' => $answer, // Placeholder
                    'type' => $autocompleteType,
                    'is_active' => true
                ]);
            } else {
                // Use the exact name from DB to avoid mismatches
                $answer = $gameItem->name_en;
            }

            // Create scrambled version
            $cleanAnswer = preg_replace('/[^a-zA-Z0-9]/', '', $answer);
            $scrambled = str_shuffle(strtoupper($cleanAnswer));
